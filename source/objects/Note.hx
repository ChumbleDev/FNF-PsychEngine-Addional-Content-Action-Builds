...
class Note extends FlxSprite
{
	...
	public var susTouch:Bool = false;
	...
	public function followStrumNote(myStrum:FlxTypedGroup<StrumNote>, fakeCrochet:Float, songSpeed:Float = 1)
	{
		var strumX:Float = myStrum.members[noteData].x;
		var strumY:Float = myStrum.members[noteData].y;
		var strumAngle:Float = myStrum.members[noteData].angle;
		var strumAlpha:Float = myStrum.members[noteData].alpha;
		var strumDirection:Float = myStrum.members[noteData].direction;

		distance = (0.45 * (Conductor.songPosition - strumTime) * songSpeed * multSpeed);
		if (!myStrum.members[noteData].downScroll) distance *= -1;
		...
		if(copyAlpha)
			alpha = strumAlpha * multAlpha + alphaOffset;
		...
		if(copyY)
		{
			...
			if(myStrum.members[noteData].downScroll && isSustainNote)
			{
				if(PlayState.isPixelStage)
				{
