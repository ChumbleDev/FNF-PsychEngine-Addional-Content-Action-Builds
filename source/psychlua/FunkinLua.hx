...
class FunkinLua {
	...
	public function new(scriptName:String) {
		...
		set('modchart', ClientPrefs.data.modchart);
		set('jukebox', ClientPrefs.data.jukebox);
		...
		Lua_helper.add_callback(lua, "setWindowX", function(x:Int) {
			Window.x = calc_pos()[0] + x;
		});
		Lua_helper.add_callback(lua, "addWindowX", function(ax:Int) {
			Window.x += ax;
		});
		Lua_helper.add_callback(lua, "setWindowY", function(y:Int) {
			Window.y = calc_pos()[1] + y;
		});
		Lua_helper.add_callback(lua, "addWindowY", function(ay:Int) {
			Window.y += ay;
		});
		Lua_helper.add_callback(lua, "setWindowXY", function(x:Int, y:Int) {
			Window.setPos(calc_pos()[0] + x, calc_pos()[1] + y);
		});
		Lua_helper.add_callback(lua, "addWindowXY", function(ax:Int, ay:Int) {
			Window.x += ax;
			Window.y += ay;
		});
		Lua_helper.add_callback(lua, "setWindowWidth", function(w:Int) {
			Window.width = w;
		});
		Lua_helper.add_callback(lua, "setWindowHeight", function(h:Int) {
			Window.height = h;
		});
		Lua_helper.add_callback(lua, "addWindowWidth", function(aw:Int) {
			Window.width += aw;
		});
		Lua_helper.add_callback(lua, "addWindowHeight", function(ah:Int) {
			Window.height += ah;
		});
		Lua_helper.add_callback(lua, "setWindowWH", function(w:Int, h:Int) {
			Window.setSize(w, h);
		});
		Lua_helper.add_callback(lua, "setWindowXYWH", function(x:Int, y:Int, w:Int, h:Int) {
			Window.setPos(calc_pos()[0] + x, calc_pos()[1] + y);
			Window.setSize(w, h);
		});
		Lua_helper.add_callback(lua, "addWindowWH", function(aw:Int, ah:Int) {
			Window.width += aw; Window.height += ah;
		});
		Lua_helper.add_callback(lua, "addWindowXYWH", function(ax:Int, ay:Int, aw:Int, ah:Int) {
			Window.x += ax; Window.y += ay;
			Window.width += aw; Window.height += ah;
		});
		Lua_helper.add_callback(lua, "setWindowTitle", function(s:String) {
			Window.title = s;
		});
		set("displayWidth", Window.dis_width);
		set("displayHeight", Window.dis_height);
		set("user_os", Window.os);
		set("user_lang", Window.sys_lang);
