...
// i dont wanna visit this file again..
...
class StageEditorState extends MusicBeatState
{
    ...
    override function create()
    {
        ...
        trace("Is From Modding Folder: " + isModFolder);
        trace("Then Base Path is: " + currentPath);
        #end
        ...
        infoTxt = new FlxText(0, FlxG.height, 0, "Press SPACE to watch how to use this!", 20);
        ...
        iTxt = new FlxText(0, 10, 0, "X: 0 / Y: 0 / Width: 0 / Height: 0 / Animated: false / Zoom: 1", 20);
        iTxt.screenCenter(X);
    ...
    override function update(elapsed:Float)
    {
        ...
        if (iTxt != null)
        {
            ...
            if (obj != null)
            {
                iTxt.text = "X: " + obj.x + " / Y: " + obj.y + " / Width: " + obj.width + " / Height: " + obj.height + " / Animated: " + Std.string(!(obj.frames != null)) + " / Zoom: " + camZoom;
            } else {
                iTxt.text = "X: N/A / Y: N/A / Width: N/A / Height: N/A / Animated: N/A / Zoom: " + camZoom;
            }
            iTxt.screenCenter(X);
        }
        ...
        if (!blockInput)
        {
            if (!isCharacterMode)
            {
                ...
                if (FlxG.keys.pressed.CONTROL)
                {
                    ...
                    if (FlxG.keys.justPressed.DOWN) spr.y += 5;
                }
        ...
        if (spr != null)
        {
            spr.alpha = 0.75 + Math.sin(elapsed2)*0.25;
        }

        if (FlxG.keys.justPressed.SPACE)
        {
            CoolUtil.browserLoad("https://github.com/Mago0643/FNF-PsychEngine-Addional-Content/blob/main/Stage%20Editor.md");
            FlxTween.cancelTweensOf(infoTxt.scale);
            infoTxt.scale.set(1.2, 1.2);
            FlxTween.tween(infoTxt.scale, {x: 1, y: 1}, 1.0, {ease: FlxEase.expoOut});
        }
        ...
        switch (num)
        {
            case "Characters":
                ...
                var gfDD = new FlxUIDropDownMenu(bfDD.x, bfDD.y+dropdownShit+25, FlxUIDropDownMenu.makeStrIdLabelArray(characters, true), function(name:String)
                {
                    ...
                    gf = new Character(0, 0, characters[Std.parseInt(name)]);
                ...
                var dadDD = new FlxUIDropDownMenu(gfDD.x, gfDD.y + dropdownShit + 25, FlxUIDropDownMenu.makeStrIdLabelArray(characters, true), function(name:String)
                {
                    ...
                    opponent = new Character(0, 0, characters[Std.parseInt(name)], false);
            ...
            case "Animation":
                ...
                {
                    var fuck = new BGSprite(animFile.text, posX.value, posY.value, scrollX.value, scrollY.value);
            ...
            case "Asset":
                ...
                sprXInput = new FlxUINumericStepper(fuck.x, fuck.y + 25 + fuck.height, 1, 0, Math.NEGATIVE_INFINITY, Math.POSITIVE_INFINITY, 2);
                ...
                var addButt = new FlxButton(fuck.x, tab.height - 50, "Add Image", function()
                {
                    {
                        path = currentPath+"/"+directoryInput.text + "/images/" + fuck.text + ".png";
                       var ok = new BGSprite(fuck.text, sprXInput.value, sprYInput.value, sprSFXInput.value, sprSFYInput.value);
                ...
                var updateButt = new FlxButton(addButt.x + addButt.width + 70, addButt.y, "Update Image", function()
                {
                    ...
                    if (target != null)
                    {
                        if (target.frames == null) // if its animated
                        {
                            ...
                            openSubState(new Prompt("This Image is Animated,\nChange To Static Image?", 0, function()
                            {
                                path = currentPath+"/"+directoryInput.text + "/images/" + fuck.text + ".png";
                                {
                                    var pos = stageGrp.members.indexOf(target);
                                    stageGrp.remove(target, true);
                                    target = new BGSprite(fuck.text, sprXInput.value, sprYInput.value, sprSFXInput.value, sprSFYInput.y);
                                    target.setGraphicSize(Std.int(sprSXInput.value), Std.int(sprSYInput.value));
                                    target.updateHitbox();
                                    target.cameras = [camStage];
                ...
                {
                    {
                        try
                        {
                            var newData = StageAsset.getStageAsset(fileInput.text);
                            trace(newData);

                            var newPos = StageData.getStageFile(fileInput.text);
                            trace(newPos);

                            if (newPos == null) newPos = StageData.dummy();
                            if (newData == null) newData = StageAsset.makeStageAsset();
                            ...
                            Paths.setCurrentLevel(data_pos.directory);
    ...
    {
        var date = Date.now();
        var path = currentPath+"/stages/asset/"+date.toString().replace(":", "-") + ".json";
        #if MODS_ALLOWED
        path = Paths.modsJson(currentPath+"/stage/"+date.toString().replace(":", "-"));
        #end
    ...
    function resetStage()
    {
        ...
        for (i in 0...data.sprites.length)
        {
            ...
            {
                var ok = new BGSprite(data.sprites[i], data.position[i][0], data.position[i][1], data.scroll[i][0], data.scroll[i][1]);
        ...
        for (i in 0...data.anim_frame.length)
        {
            ...
            {
                var sparrow = new BGSprite(data.anim_frame[i], data.position[i][0], data.position[i][1], data.scroll[i][0], data.scroll[i][1], data.anim_n[i], data.anim_loop[i]);
                sparrow.setGraphicSize(data.sizes[i][0], data.sizes[i][1]);
