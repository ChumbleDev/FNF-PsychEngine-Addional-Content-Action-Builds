...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Set Camera Zoom', "Sets A Camera Zoom that is not same as Add Camera Zoom.\nValue 1: Camera Zoom."],
		['Cinematic Bar', "Shows A Cinematic Bar.\nValue 1: The Time Bar Moves.\nValue 2: How much The Bar should Move?"]
	];
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var sv_cheats = new FlxUICheckBox(stepperBPM.x + stepperBPM.width + 10, stepperBPM.y, null, null, "Only Modchart", 100);
		sv_cheats.checked = _song.modchart;
		sv_cheats.callback = function()
		{
			_song.modchart = sv_cheats.checked;
		};
	...
	var beatlineStepper:FlxUINumericStepper;
	function addChartingUI() {
		...
		// note - you can make this like decimals by typing
		beatlineStepper = new FlxUINumericStepper(waveformUseVoices.x, mouseScrollingQuant.y, 1, 4, 2, 8, 1);
		beatlineStepper.value = FlxG.save.data.chart_beatline;
		beatlineStepper.name = "beatline_number";
		blockPressWhileTypingOnStepper.push(beatlineStepper);
		...
		tab_group_chart.add(new FlxText(beatlineStepper.x, beatlineStepper.y - 15, 0, "Measure Line Count:"));
		tab_group_chart.add(metronome);
		tab_group_chart.add(disableAutoScrolling);
		tab_group_chart.add(metronomeStepper);
		tab_group_chart.add(beatlineStepper);
	...
	var chartInfo:FlxUIInputText;
	var artistInfo:FlxUIInputText;
	function addDataUI()
	{
		...
		chartInfo = new FlxUIInputText(check_disableNoteRGB.x, check_disableNoteRGB.y + check_disableNoteRGB.height + 25, 150, "unknown", 8);
		blockPressWhileTypingOn.push(chartInfo);

		artistInfo = new FlxUIInputText(chartInfo.x, chartInfo.y + chartInfo.height + 25, 150, "???", 8);
		blockPressWhileTypingOn.push(artistInfo);
		...
		tab_group_data.add(new FlxText(chartInfo.x, chartInfo.y - 20, 0, "Charter Name:"));
		tab_group_data.add(new FlxText(artistInfo.x, artistInfo.y - 20, 0, "Song Artist:"));
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			switch(wname)
			{
				...
				case 'beatline_number':
					FlxG.save.data.chart_beatline = nums.value;
					reloadGridLayer();
			...
			else if (sender == chartInfo) {
				_song.charter = chartInfo.text;
			}
			else if (sender == artistInfo) {
				_song.artist = artistInfo.text;
			}
	...
	override function update(elapsed:Float)
	{
		...
		var quantShit = "th";
		if (quantization == 32 || quantization == 192) quantShit = "nd"; // this should fix

		// i suck at codes
		var beatShit = Std.string(CoolUtil.floorDecimal(curDecBeat, 3));
		if (beatShit.indexOf(".") == -1)
			// add decimals
			beatShit += ".000";
		else {
			var fazbear = beatShit.split(".");
			if (fazbear[1].length < 3) {
				for (i in 0...fazbear[1].length) // add zero
					fazbear[1] += "0";
				beatShit = fazbear[0] + "." + fazbear[1];
			}
		}

		bpmTxt.text =
		FlxStringUtil.formatTime(FlxG.sound.music.time / 1000, true) + " / " + FlxStringUtil.formatTime(FlxG.sound.music.length / 1000, true) +
		"\nSection: " + curSec +
		"\n\nBeat: " + beatShit +
		"\n\nStep: " + curStep +
		"\n\nBeat Snap: " + quantization + quantShit;
	...
	*/
	...
	function reloadGridLayer() {
		...
		if (FlxG.save.data.chart_beatline == null) FlxG.save.data.chart_beatline = 4;
		if (FlxG.save.data.chart_beatline <= 1) FlxG.save.data.chart_beatline = 2;
		if (FlxG.save.data.chart_beatline > 8) FlxG.save.data.chart_beatline = 8;
		for (i in 1...16) {
			var beatsep:FlxSprite = new FlxSprite(gridBG.x, (GRID_SIZE * ((FlxG.save.data.chart_beatline / 2) * curZoom)) * i).makeGraphic(1, 1, 0xFF0000FF);
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			{
				curRenderedSustains.add(setupSusNote(note, beats, false));
				if (!PlayState.isPixelStage) curRenderedSustains.add(setupSusNote(note, beats, true));
			}
		...
		if(curSec < _song.notes.length-1) {
			for (i in _song.notes[curSec+1].sectionNotes)
			{
				...
				if (note.sustainLength > 0)
				{
					nextRenderedSustains.add(setupSusNote(note, beats, false));
					if (!PlayState.isPixelStage) nextRenderedSustains.add(setupSusNote(note, beats, true));
				}
			}
		}
	...
	function setupSusNote(note:Note, beats:Float, ?isEnd:Bool = false):FlxSprite {
		...
		var colorSet = PlayState.isPixelStage ? ClientPrefs.data.arrowRGBPixel : ClientPrefs.data.arrowRGB;
		var spr = new FlxSprite(note.x + (GRID_SIZE * 0.5) - 8, note.y + GRID_SIZE / 2);
		var colorShad:RGBShaderReference;
		if (!PlayState.isPixelStage) {
			var tex = note.texture;
			if (tex.length < 1 || tex == null)
			{
				tex = _song.arrowSkin;
				if (tex.length < 1 || tex == null) tex = Note.defaultNoteSkin.split("/")[1];
			}
			spr.frames = Paths.getSparrowAtlas("noteSkins/"+tex+Note.getNoteSkinPostfix(), "shared");
			spr.animation.addByPrefix(Note.colArray[note.noteData] + 'hold', Note.colArray[note.noteData] + ' hold piece', 24, true);
			spr.animation.addByPrefix('purpleholdend', 'pruple end hold', 24, true);
			spr.animation.addByPrefix(Note.colArray[note.noteData] + 'holdend', Note.colArray[note.noteData] + ' hold end', 24, true);
			if (isEnd) {
				spr.y += height;
				spr.animation.play(Note.colArray[note.noteData] + 'holdend', true);
				spr.setGraphicSize(16, Std.int(spr.height*0.125));
			} else {
				spr.animation.play(Note.colArray[note.noteData] + 'hold', true);
				spr.setGraphicSize(16, height);
			}
		} else {
			spr.makeGraphic(8, height);
			spr.color = colorSet[note.noteData % 4][2];
			spr.x += 4;
		}
		spr.updateHitbox();
		if (!_song.disableNoteRGB)
		{
			var rgbShit:RGBPalette = new RGBPalette();
			rgbShit.r = colorSet[note.noteData][0];
			rgbShit.g = colorSet[note.noteData][1];
			rgbShit.b = colorSet[note.noteData][2];
			colorShad = new RGBShaderReference(spr, rgbShit);
		}
		return spr;
	}
