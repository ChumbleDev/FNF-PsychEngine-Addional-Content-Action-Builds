...
import backend.ShaderCamera;
import backend.Achievements;
import backend.Highscore;
import backend.Section.SwagSection;
import backend.WeekData;
import backend.StageData;
import backend.EaseConvert;
...
class PlayState extends MusicBeatState
{
	...
	public static var judResult:Array<Int> = [
		0, // Sick!!
		0, // Good
		0, // Bad
		0, // Shit
	...
	// you died in story mode, then should recover last song jud results.
	public static var storyJud:Array<Int> = [0,0,0,0,0];
	...
	public static var strumLineY:Float = 0;
	...
	public var vaildScore:Bool = true;
	var zoo:Float = 0;
	var bar:CinematicBar;
	var judTxt:FlxTypedGroup<FlxText>;
	//now i love using get() functions
	public var notitg(get,never):Bool;
	function get_notitg() return (SONG != null ? SONG.modchart : false);

	// Callbacks for stages
	public var startCallback:Void->Void = null;
	public var endCallback:Void->Void = null;

	// i fucked these somehow
	public static function checkStage():backend.BaseStage {
		switch (PlayState.curStage)
		{
			case 'stage': return new states.stages.StageWeek1(); //Week 1
			case 'spooky': return new states.stages.Spooky(); //Week 2
			case 'philly': return new states.stages.Philly(); //Week 3
			case 'limo': return new states.stages.Limo(); //Week 4
			case 'mall': return new states.stages.Mall(); //Week 5 - Cocoa, Eggnog
			case 'mallEvil': return new states.stages.MallEvil(); //Week 5 - Winter Horrorland
			case 'school': return new states.stages.School(); //Week 6 - Senpai, Roses
			case 'schoolEvil': return new states.stages.SchoolEvil(); //Week 6 - Thorns
			case 'tank': return new states.stages.Tank(); //Week 7 - Ugh, Guns, Stress
		}
		return null;
	}

	public function checkStageLua() {
		#if LUA_ALLOWED
		return startLuasNamed('stages/' + curStage + '.lua');
		#end
	}

	public function checkStageHScript() {
		#if HSCRIPT_ALLOWED
		return startHScriptsNamed('stages/' + curStage + '.hx');
		#end
	}

	public var shad:ShaderCamera = new ShaderCamera();
	public var modchart(get,never):Bool;
	function get_modchart() {return (ClientPrefs.data.modchart && EngineSettings.modcharts);}
	public var jukebox(get,never):Bool;
	function get_jukebox() {return (!chartingMode && ClientPrefs.data.jukebox && EngineSettings.jukeboxMode);}
	...
	override public function create()
	{
		...
		// the annoying botplay disable on charting mode
		if (!chartingMode) cpuControlled = ClientPrefs.getGameplaySetting('botplay');
		...
		for (cam in [camGame, camHUD, camOther])
		{
			cam.setFilters(shad.getShader());
		}
		...
		if (notitg) {
			ClientPrefs.data.modchart = true;
			ClientPrefs.data.jukebox = false;
		}
		...
		// STAGE SCRIPTS
		// doing !notitg to reduce loading at modchart only charts
		if (!notitg) checkStageLua();
		checkStageHScript();
		...
		timeTxt.setFormat(EngineSettings.defaultFont, 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		timeBar = new HealthBar(0, timeTxt.y + (timeTxt.height / 4), 'timeBar', null, function() return songPercent, 0, 1);
		timeBar.scrollFactor.set();
		timeBar.screenCenter(X);
		timeBar.alpha = 0;
		timeBar.visible = showTime;

		bar = new CinematicBar(200);
		bar.lowerBar.y = FlxG.height;
		bar.upperBar.y = -200;
		add(bar);
		...
		if (!modchart) add(grpNoteSplashes);
		...
		scoreTxt.setFormat(EngineSettings.defaultFont, 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreTxt.scrollFactor.set();
		scoreTxt.borderSize = 1.25;
		scoreTxt.visible = !ClientPrefs.data.hideHud;
		add(scoreTxt);

		if (jukebox) cpuControlled = true;

		botplayTxt = new FlxText(400, timeBar.y + 55, FlxG.width - 800, jukebox ? "JUKEBOX" : "BOTPLAY", 32);
		botplayTxt.setFormat(EngineSettings.defaultFont, 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		judTxt = new FlxTypedGroup<FlxText>();
		add(judTxt);

		var col = [FlxColor.fromRGBFloat(0,1,1), FlxColor.fromRGBFloat(0,1,0), FlxColor.fromRGBFloat(1,0.5,0), FlxColor.fromRGBFloat(0.75,0,0)];
		for (i in 1...5) {
			var txt = new FlxText(10,0, 0, "0");
			txt.screenCenter(Y);
			txt.setFormat(EngineSettings.defaultFont, 22, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			txt.y += i * txt.height;
			txt.color = col[i-1];
			txt.visible = !jukebox;
			judTxt.add(txt);
		...
		if (notitg) {
			iconP1.visible = false;
			iconP2.visible = false;
			healthBar.setColors(0xFF000000, 0xFF0000FF);
			// doing this cuz i dont want to do "if (notitg)" to the all places
			...
			var cover = new FlxSprite().makeGraphic(5000, 5000, FlxColor.BLACK);
			cover.screenCenter();
			add(cover);
		}
		...
		#if LUA_ALLOWED
		var isModFolder:Bool = !FileSystem.isDirectory("assets/data/"+songName+"/");
		if (isModFolder) {
			...
			for (file in FileSystem.readDirectory(Paths.getPath("data/"+songName+"/")))
			{
				if(file.toLowerCase().endsWith('.lua'))
					new FunkinLua(Paths.getPath("data/"+songName+"/") + file);
				if(file.toLowerCase().endsWith('.hx'))
					initHScript(Paths.getPath("data/"+songName+"/") + file);
			}
		}
		#end
		...
		if (modchart) ModchartFuncs.loadLuaFunctions();
	...
	public function startCountdown()
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			if (modchart) NoteMovement.getDefaultStrumPos(this);
	...
	inline private function createCountdownSprite(image:String, antialias:Bool):FlxSprite
	{
		...
		doTween(spr, {/*y: spr.y + 100,*/ alpha: 0}, Conductor.crochet / 1000, {
	...
	function startSong():Void
	{
		...
		doTween(timeBar, {alpha: 1}, 0.5, {ease: FlxEase.circOut});
		doTween(timeTxt, {alpha: 1}, 0.5, {ease: FlxEase.circOut});
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...4)
		{
			...
			if (isStoryMode && skipArrowStartTween) babyArrow.alpha = targetAlpha;
	...
	override function openSubState(SubState:FlxSubState)
	{
		...
		if (paused)
		{
			...
			var stat = false;
			#if LUA_ALLOWED
			for (tween in modchartTweens) tween.active = stat;
			for (timer in modchartTimers) timer.active = stat;
			#end

			for (tweens in curTweens) tweens.active = stat;
			if (scoreTxtTween != null) scoreTxtTween.active = stat;
			if (songSpeedTween != null) songSpeedTween.active = stat;
			if (cameraTwn != null) cameraTwn.active = stat;
		}
	...
	override public function update(elapsed:Float)
	{
		...
		var lol = ['Sick: ${judResult[0]}', 'Good: ${judResult[1]}', 'Bad: ${judResult[2]}', 'Shit: ${judResult[3]}'];
		for (i in 0...judTxt.length) {
			judTxt.members[i].text = lol[i];
		}
		...
		if (camZooming)
		{
			FlxG.camera.zoom = FlxMath.lerp(defaultCamZoom + zoo, FlxG.camera.zoom, FlxMath.bound(1 - (elapsed * 3.125 * camZoomingDecay * playbackRate), 0, 1));
				...
				if(notes.length > 0)
				{
					if(startedCountdown)
					{
						...
						notes.forEachAlive(function(daNote:Note)
						{
							...
							daNote.followStrumNote(strumGroup, fakeCrochet, songSpeed / playbackRate);
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
		...
		switch(eventName) {
			...
			case 'Set Camera Zoom':
				zoo = flValue1;
			case 'Cinematic Bar':
				doTween(bar.upperBar, {y: Std.parseFloat(value2) - bar.bar_height}, flValue1, {ease: FlxEase.expoOut});
				doTween(bar.lowerBar, {y: FlxG.height - Std.parseFloat(value2) - bar.bar_height}, flValue2, {ease: FlxEase.expoOut});
		}
	...
	public function endSong()
	{
		...
		{
			if (jukebox && !isStoryMode) {
				var songAry:Array<String> = [];
				for (song in FileSystem.readDirectory("assets/data/"))
				{
					// exclude files
					if (!song.contains(".")) {
						songAry.push(song);
				...
				var songName = FlxG.random.getObject(songAry);
				var diff = "";
				if (Difficulty.getString().toLowerCase() != Difficulty.getDefault().toLowerCase()) diff = "-" + Difficulty.getString().toLowerCase();
				var ssss =Song.loadFromJson(songName + diff, songName);
				StageData.loadDirectory(ssss);
				Paths.setCurrentLevel(StageData.forceNextDirectory);
				trace("Resulting " + songAry + ", Picking " + songName);
				SONG = ssss;
				FlxG.sound.music.stop();
			...
			if (vaildScore && !ClientPrefs.getGameplaySetting('practice') && !ClientPrefs.getGameplaySetting('botplay') && !usedBotplay) Highscore.saveScore(SONG.song, songScore, storyDifficulty, percent);
			#end
			playbackRate = 1;
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					//MusicBeatState.switchState(new ScoreResultState(new StoryMenuState()));
					MusicBeatState.switchState(new StoryMenuState());
					judResult = [0, 0, 0, 0, 0];

					// if ()
					if(!ClientPrefs.getGameplaySetting('practice') && !ClientPrefs.getGameplaySetting('botplay') && !usedBotplay && vaildScore) {
			...
			else
			{
				...
				judResult = [0, 0, 0, 0, 0];
				//MusicBeatState.switchState(new ScoreResultState(new FreeplayState()));
	...
	// you can animate the score/combos now!!
	private function popUpScore(note:Note = null):Void
	{
		...
		switch (daRating.name.toLowerCase()) {
			case "sick": judResult[0] += 1;
			case "good": judResult[1] += 1;
			case "bad": judResult[2] += 1;
			case "shit": judResult[3] += 1;
		}
		...
		for (i in seperatedScore)
		{
			...
			doTween(numScore, {alpha: 0}, 0.2 / playbackRate, {
		...
		doTween(rating, {alpha: 0}, 0.2 / playbackRate, {
			startDelay: Conductor.crochet * 0.001 / playbackRate
		});

		doTween(comboSpr, {alpha: 0}, 0.2 / playbackRate, {
			onComplete: function(tween:FlxTween)
			{
				comboSpr.destroy();
				rating.destroy();
	...
	function between(g:Float, f:Float, e:Float):Bool {
		return g >= f && g <= e;
	}
	...
	override function stepHit()
	{
		...
		switch (SONG.song.toLowerCase())
        {
            case "blammed":
                ...
                {
                    case 112 | 496:
                        doTween(shad, {fisheye: -0.3, glitch: 0.0075}, Conductor.stepCrochet / 1000 * 16, {ease: FlxEase.quadIn, onComplete: function(s)
							{
								doTween(shad, {fisheye: 0, glitch: 0}, Conductor.crochet / 1000 * 2, {ease: FlxEase.expoOut});
							}
						});
                }
        }
	...
	override function beatHit()
	{
		...
		{
			case "stress":
				if (between(curBeat, 224, 254))
				{
					FlxG.camera.zoom += 0.015;
					camHUD.zoom += 0.05;
				}
            case "blammed":
                if (between(curBeat, 32, 96) && curBeat+2 % 4 == 0)
                {
                    FlxG.camera.zoom += 0.015;
					camHUD.zoom += 0.05;
                }

                if (between(curBeat, 130, 192))
                {
                	FlxG.camera.zoom += 0.015;
                	camHUD.zoom += 0.05;
					if (curBeat % 4 == 0)
					{
						shad.fisheye = -0.15;
						doTween(shad, {fisheye: 0.0}, Conductor.crochet / 1000 * 4, {ease: FlxEase.quadOut});
                	}
				}
		}
	...
	public function startLuasNamed(luaFile:String)
	{
		...
		{
			for (script in luaArray)
				if(script.scriptName == luaToLoad) return null;
	
			return new FunkinLua(luaToLoad);
		}
		return null;
	}
	#end
	...
	public function startHScriptsNamed(scriptFile:String)
	{
		...
		if(FileSystem.exists(scriptToLoad))
		{
			if (SScript.global.exists(scriptToLoad)) return null;
	
			return initHScript(scriptToLoad);
		}
		return null;
	}
