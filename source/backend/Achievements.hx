...
class Achievements {
	public static var achievementsStuff:Array<Dynamic> = [ //Name, Description, Achievement save tag, Hidden achievement
		...
		#if ENABLE_SHITS ,["April Fools!", "Play on April Fools.", 'aprilfools', true] #end
	];
	...
	/**
		Checks the achievement by name and gives it
	**/
	public static function checkAchievement(current:MusicBeatState, name:String, ?camera:Null<FlxCamera> = null) {
		var id = getAchievementIndex(name);
		if (!isAchievementUnlocked(achievementsStuff[id][2])) {
			achievementsMap.set(achievementsStuff[id][2], true);
			current.add(new AchievementPopup(name, camera));
			FlxG.sound.play(Paths.sound('confirmMenu'), 0.7);
	...
	/**
		Checks the achievement by name and gives it (in substate)
	**/
	public static function checkAchievementSub(current:MusicBeatSubstate, name:String, ?camera:Null<FlxCamera> = null) {
		var id = getAchievementIndex(name);
		if (!isAchievementUnlocked(achievementsStuff[id][2])) {
			achievementsMap.set(achievementsStuff[id][2], true);
			current.add(new AchievementPopup(name, camera));
