...
class NoteOffsetState extends MusicBeatState
{
	...
	function createStrum(player:Int) {
		var strumLineX:Float = ClientPrefs.data.middleScroll ? PlayState.STRUM_X_MIDDLESCROLL : PlayState.STRUM_X;
		var strumLineY:Float = ClientPrefs.data.downScroll ? (FlxG.height - 150) : 50;
		var strumWidth:Float = 0;
		for (i in 0...4) {
			var arrow = new StrumNote(strumLineX, strumLineY, i, player);
			arrow.downScroll = ClientPrefs.data.downScroll;
			var targetAlpha:Float = 1;
			if (player < 1) {
				if(!ClientPrefs.data.opponentStrums) targetAlpha = 0;
				else if(ClientPrefs.data.middleScroll) targetAlpha = 0.35;
			}
			arrow.offsetAlpha = targetAlpha;
			if (player != 1) {
				if(ClientPrefs.data.middleScroll) {
					arrow.x += 310;
					if(i > 1) { //Up and Right
						arrow.x += FlxG.width / 2 + 25;
					}
				}
			}
			...
			strumWidth = arrow.width;
		}

		switch (player) {
			case 0:
				leftSide = new FlxSprite(strumLineX + 50).makeGraphic(Std.int(strumWidth*4), FlxColor.RED);
				leftSide.alpha = 0.25;
				add(leftSide);
			case 1:
				rightSide = new FlxSprite(strumLineX + 50 + (FlxG.width / 2)).makeGraphic(Std.int(strumWidth*4), FlxColor.LIME);
				rightSide.alpha = 0.25;
				add(rightSide);
		}
	}
	...
	function updateMode()
	{
		...
		leftSide.visible = onComboMenu;
		rightSide.visible = onComboMenu;
