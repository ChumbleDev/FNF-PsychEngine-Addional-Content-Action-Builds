...
class BeatXModifier extends Modifier
{
    ...
    public static function getShift(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int):Float
    {
        ...
        * acceleration to compensate or it'll look weird. */
        ...
        var fShift = 20.0*fAmount*FlxMath.fastSin( (curPos *0.01) + MathUtil.PI_DEVIDE_2 );
        return fShift;
    }
}
...
// maybe some mods are not work with other engines..
...
class TanDrunkXModifier extends Modifier
{
    ...
    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
    {
        noteData.x += currentValue * (MathUtil.fastTan( ((Conductor.songPosition*0.001) + ((lane%NoteMovement.keyCount)*0.2) + 
        (curPos*0.45)*(10/FlxG.height)) * (subValues.get('speed').value*0.2)) * Note.swagWidth*0.5);
    }
    override function strumMath(noteData:NotePositionData, lane:Int, pf:Int)
    {
...
class TanDrunkYModifier extends Modifier
{
    ...
    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
    {
        noteData.y += currentValue * (MathUtil.fastTan( ((Conductor.songPosition*0.001) + ((lane%NoteMovement.keyCount)*0.2) + 
        (curPos*0.45)*(10/FlxG.height)) * (subValues.get('speed').value*0.2)) * Note.swagWidth*0.5);
    }
    override function strumMath(noteData:NotePositionData, lane:Int, pf:Int)
    {
...
class TanDrunkZModifier extends Modifier
{
    ...
    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
    {
        noteData.z += currentValue * (MathUtil.fastTan( ((Conductor.songPosition*0.001) + ((lane%NoteMovement.keyCount)*0.2) + 
        (curPos*0.45)*(10/FlxG.height)) * (subValues.get('speed').value*0.2)) * Note.swagWidth*0.5);
    }
    override function strumMath(noteData:NotePositionData, lane:Int, pf:Int)
    {
...
class TanTipsyXModifier extends Modifier
{
    override function setupSubValues()
    {
        subValues.set('speed', new ModifierSubValue(1.0));
    ...
    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
    {
        noteData.x += currentValue * ( MathUtil.fastTan( Conductor.songPosition*0.001 *(1.2) + 
        (lane%NoteMovement.keyCount)*(2.0) + subValues.get('speed').value*(0.2) ) * Note.swagWidth*0.4 );
    }
    override function strumMath(noteData:NotePositionData, lane:Int, pf:Int)
    {
...
class TanTipsyYModifier extends Modifier
{
    override function setupSubValues()
    {
        subValues.set('speed', new ModifierSubValue(1.0));
    ...
    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
    {
        noteData.y += currentValue * ( MathUtil.fastTan( Conductor.songPosition*0.001 *(1.2) + 
        (lane%NoteMovement.keyCount)*(2.0) + subValues.get('speed').value*(0.2) ) * Note.swagWidth*0.4 );
    }
    override function strumMath(noteData:NotePositionData, lane:Int, pf:Int)
    {
...
class TanTipsyZModifier extends Modifier
{
    override function setupSubValues()
    {
        subValues.set('speed', new ModifierSubValue(1.0));
    ...
    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
    {
        noteData.z += currentValue * ( MathUtil.fastTan( Conductor.songPosition*0.001 *(1.2) + 
        (lane%NoteMovement.keyCount)*(2.0) + subValues.get('speed').value*(0.2) ) * Note.swagWidth*0.4 );
    }
    override function strumMath(noteData:NotePositionData, lane:Int, pf:Int)
    {
...
class TanBumpyXModifier extends Modifier 
{
    ...
    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
    {
        noteData.x += currentValue * 40 * MathUtil.fastTan(curPos*0.01*subValues.get('speed').value);
    }
}
...
class TanBumpyYModifier extends Modifier 
{
    ...
    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
    {
        noteData.y += currentValue * 40 * MathUtil.fastTan(curPos*0.01*subValues.get('speed').value);
    }
}
...
class TanBumpyModifier extends Modifier 
{
    ...
    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
    {
        noteData.z += currentValue * 40 * MathUtil.fastTan(curPos*0.01*subValues.get('speed').value);
    }
}

// these thing dosen't work
/*class TornadoZModifier extends Modifier
{
    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
    {
        noteData.z += TornadoModifier.doMath(lane, currentValue, curPos, renderer);
    }
}
...
class TornadoYModifier extends Modifier
{
    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
    {
        noteData.y += TornadoModifier.doMath(lane, currentValue, curPos, renderer);
    }
}
...
class TanTornadoModifier extends Modifier
{
    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
    {
        noteData.x += TornadoModifier.doMath(lane, currentValue, curPos, renderer, true);
    }
}
...
class TanTornadoYModifier extends Modifier
{
    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
    {
        noteData.y += TornadoModifier.doMath(lane, currentValue, curPos, renderer, true);
    }
}
...
class TanTornadoZModifier extends Modifier
{
    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
    {
        noteData.z += TornadoModifier.doMath(lane, currentValue, curPos, renderer, true);
    }
}

class TornadoModifier extends Modifier
{
    public static function doMath(lane:Int, curValue:Float, curPos:Float, renderer:PlayfieldRenderer, ?isTan:Bool = false)
    {
        var keyCount:Int = NoteMovement.keyCount;
        // this will be work maybe???
        var fXOffset = NoteMovement.arrowSize * ((keyCount-(lane%keyCount))/2);
        // TRICKY: Tornado is very unplayable in doubles, so use a smaller
		// tornado width if there are many columns
		var bWideField:Bool = keyCount > 4;
		var iTornadoWidth:Int = bWideField ? 2 : 3;

		var iStartCol:Int = lane - iTornadoWidth;
		var iEndCol:Int = lane + iTornadoWidth;
        // CLAMP
		iStartCol = Std.int(FlxMath.bound( iStartCol, 0, keyCount ));
		iEndCol = Std.int(FlxMath.bound( iEndCol, 0, keyCount ));

		var fMinX:Float = FlxMath.MIN_VALUE_FLOAT;
		var fMaxX:Float = FlxMath.MAX_VALUE_FLOAT;

		for( i in 0...iStartCol+1 )
		{
			fMinX = Math.min( fMinX, fXOffset );
			fMaxX = Math.max( fMaxX, fXOffset );
		}

		var fRealPixelOffset:Float = fXOffset;
		var fPositionBetween:Float = FlxMath.remapToRange( fRealPixelOffset, fMinX, fMaxX, -1, 1 );
		var fRads:Float = Math.acos( fPositionBetween );
		fRads += (-curPos / renderer.getCorrectScrollSpeed()) * 6 / FlxG.height;
		
		var fAdjustedPixelOffset:Float = FlxMath.remapToRange( !isTan ? FlxMath.fastCos(fRads) : MathUtil.fastTan(fRads), -1, 1, fMinX, fMaxX );

		return (fAdjustedPixelOffset - fRealPixelOffset) * curValue;
    }

    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
    {
        noteData.x += doMath(lane, currentValue, curPos, renderer);
    }
}*/
...
class DizzyModifier extends Modifier
{
    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
    {
        if (currentValue == 0) noteData.angle = 0;
        else {
            /* offset by VisualDelayEffect seconds */

		    var fSongBeat:Float = PlayState.instance != null ? PlayState.instance.curDecBeat : 0;
		    var fDizzyRotation:Float = curPos - fSongBeat;
		    fDizzyRotation *= currentValue;
		    fDizzyRotation = MathUtil.fmod( fDizzyRotation, MathUtil.PI_TIMES_2 );
		    fDizzyRotation *= 180/Math.PI;
		    noteData.angle = fDizzyRotation;
        }
    }
}
/*
...
{
    public static function getCenterLine():Float
    {
        if (PlayState.instance == null) return 0;
        var fMiniPercent:Float = 0;
        for (key => value in PlayState.instance.playfieldRenderer.modifiers.keyValueIterator())
        {
            if (value is MiniModifier || value is ZModifier) {
                fMiniPercent += value.currentValue;
            }
        }
        var fZoom = 1 - fMiniPercent*0.5;
        var centerLine = (FlxG.height / 2) + (ClientPrefs.data.downScroll ? -50 : 50);
        return centerLine / fZoom;
    }

    public static function getHiddenSus():Float
    {
        if (PlayState.instance == null) return 0;
        var fVisiblePercent:Float = 1;
        for (key => value in PlayState.instance.playfieldRenderer.modifiers.keyValueIterator())
        {
            if (value is SuddenModifier || value is HiddenModifier)
            {
                fVisiblePercent *= value.currentValue;
            }
        }
        return fVisiblePercent;
    }

    function getHiddenStartLine()
    {
        return getCenterLine() + 
            subValues.get("fadeDist").value * FlxMath.remapToRange( getHiddenSus(), 0, 1, 0, -0.25 ) +
            getCenterLine() * subValues.get("offset").value;
    }

    function getHiddenEndLine()
    {
        return getCenterLine() +
            subValues.get("fadeDist").value * FlxMath.remapToRange( getHiddenSus(), 0, 1, -1, -1.25 ) +
            getCenterLine() * subValues.get("offset").value;
    }
    ...
    override function setupSubValues()
    {
        ...
        subValues.set('fadeDist', new ModifierSubValue(40.0));
    }

    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
    {
        var fHiddenVisibleAdjust:Float = FlxMath.remapToRange( curPos, getHiddenStartLine(), getHiddenEndLine(), 0, -1 );
		fHiddenVisibleAdjust = FlxMath.bound( fHiddenVisibleAdjust, -1, 0 );
		noteData.alpha *= FlxMath.bound(currentValue * fHiddenVisibleAdjust, 0, 1);
    }    
}
...
class SuddenModifier extends Modifier
{
    ...
    {
        return HiddenModifier.getCenterLine() + 
            subValues.get("fadeDist").value * FlxMath.remapToRange( HiddenModifier.getHiddenSus(), 0, 1, 1, 1.25 ) +
            HiddenModifier.getCenterLine() * subValues.get("offset").value;
    }

    function getSuddenEndLine()
    {
        return HiddenModifier.getCenterLine() + 
            subValues.get("fadeDist").value * FlxMath.remapToRange( HiddenModifier.getHiddenSus(), 0, 1, 0, 0.25 ) +
            HiddenModifier.getCenterLine() * subValues.get("offset").value;
    }

    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
    {
        var fHiddenVisibleAdjust:Float = FlxMath.remapToRange( curPos, getSuddenStartLine(), getSuddenEndLine(), -1, 0 );
        fHiddenVisibleAdjust = FlxMath.bound( fHiddenVisibleAdjust, -1, 0 );
        noteData.alpha *= FlxMath.bound(currentValue * fHiddenVisibleAdjust, 0, 1);
    }
}*/