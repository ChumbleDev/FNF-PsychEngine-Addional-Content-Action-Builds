...
class ModchartFuncs
{
    public static function loadLuaFunctions()
    {
        ...
        #if LUA_ALLOWED
        /*#if hscript
        for (file in FileSystem.readDirectory("assets/data/"+PlayState.SONG.song.toLowerCase().replace(" ","-")+"/")) {
            if (file.toLowerCase().endsWith(".hx"))
                PlayState.instance.initHScript(file+".hx");
        }
        #end*/
        if (PlayState.instance.luaArray.length > 0) {
            for (funkin in PlayState.instance.luaArray)
            {
                ...
                Lua_helper.add_callback(funkin.lua, "initMods", function() {
                    startMod("drunk", "DrunkXModifier"); startMod("drunkP1", "DrunkXModifier", "Opponent"); startMod("drunkP2", "DrunkXModifier", "Player");
                    startMod("drunky", "DrunkYModifier"); startMod("drunkyP1", "DrunkYModifier", "Opponent"); startMod("drunkyP2", "DrunkYModifier", "Player");
                    startMod("drunkz", "DrunkZModifier"); startMod("drunkzP1", "DrunkZModifier", "Opponent"); startMod("drunkzP2", "DrunkZModifier", "Player");
                    startMod("tipsy", "TipsyYModifier"); startMod("tipsyP1", "TipsyYModifier", "Opponent"); startMod("tipsyP2", "TipsyYModifier", "Player");
                    startMod("tipsyx", "TipsyXModifier"); startMod("tipsyxP1", "TipsyXModifier", "Opponent"); startMod("tipsyxP2", "TipsyXModifier", "Player");
                    startMod("tipsyz", "TipsyZModifier"); startMod("tipsyzP1", "TipsyZModifier", "Opponent"); startMod("tipsyzP2", "TipsyZModifier", "Player");
                    startMod("reverse", "ReverseModifier"); startMod("reverseP1", "ReverseModifier", "Opponent"); startMod("reverseP2", "ReverseModifier", "Player");
                    startMod("x", "XModifier"); startMod("y", "YModifier"); startMod("z", "ZModifier");
                    startMod("xP1", "XModifier", "Opponent"); startMod("yP1", "YModifier", "Opponent"); startMod("zP1", "ZModifier", "Opponent");
                    startMod("xP2", "XModifier", "Player"); startMod("yP2", "YModifier", "Player"); startMod("zP2", "ZModifier", "Player");
                    startMod("tinyx", "ScaleXModifier"); startMod("tinyxP1", "ScaleXModifier", "Opponent"); startMod("tinyxP2", "ScaleXModifier", "Player");
                    startMod("tinyy", "ScaleYModifier"); startMod("tinyyP1", "ScaleYModifier", "Opponent"); startMod("tinyyP2", "ScaleYModifier", "Player");
                    startMod("tiny", "ScaleModifier"); startMod("tinyP1", "ScaleModifier", "Opponent"); startMod("tinyP2", "ScaleModifier", "Player");
                    startMod("flip", "FlipModifier"); startMod("flipP1", "FlipModifier", "Opponent"); startMod("flipP2", "FlipModifier", "Player");
                    startMod("invert", "InvertModifier"); startMod("invertP1", "InvertModifier", "Opponent"); startMod("invertP2", "InvertModifier", "Player");
                    startMod("beat", "BeatXModifier"); startMod("beatP1", "BeatXModifier", "Opponent"); startMod("beatP2", "BeatXModifier", "Player");
                    startMod("beaty", "BeatYModifier"); startMod("beatyP1", "BeatYModifier", "Opponent"); startMod("beatyP2", "BeatYModifier", "Player");
                    startMod("beatz", "BeatZModifier"); startMod("beatzP1", "BeatZModifier", "Opponent"); startMod("beatzP2", "BeatZModifier", "Player");

                    for (i in 0...8) {
                        startMod("reverse"+i, "ReverseModifier", "Lane");
                        startMod("x"+i, "XModifier", "Lane"); startMod("y"+i, "YModifier", "Lane"); startMod("z"+i, "ZModifier", "Lane");
                        startMod("tiny"+i, "ScaleModifier", "Lane"); startMod("tinyx"+i, "ScaleXModifier", "Lane"); startMod("tinyy"+i, "ScaleYModifier", "Lane");
                        setModTargetLane("reverse"+i, i);
                        setModTargetLane("x"+i, i); setModTargetLane("y"+i, i); setModTargetLane("z"+i, i);
                        setModTargetLane("tiny"+i, i); setModTargetLane("tinyx"+i, i); setModTargetLane("tinyy"+i, i);
                    }

                    // idk add it yourself if you want

                    PlayState.instance.playfieldRenderer.modifierTable.reconstructTable();
                });
                // nvm doing it later
                // making it like mirin template
                // set {0, 50, "drunkx", 0, "drunky"}
                /*Lua_helper.add_callback(funkin.lua, 'set', function(table:Array<Dynamic>) {
                    if (!Std.isOfType(table[0], Float)) FunkinLua.luaTrace("Mods Error: Beat should be Float!", false, false, FlxColor.RED); return;
                    var beat:Float = Std.parseFloat(Std.string(table[0]));
                    var fuck:Array<Dynamic> = [];

                    for (index in 1...table.length) {
                        var noMods = false;
                        // mod string
                        if (index % 2 == 0) {
                            if (!Std.isOfType(table[index], String)) FunkinLua.luaTrace("Mods Error: Mod name should be String!", false, false, FlxColor.RED); noMods=true; break; return;
                            fuck.push(Std.string(table[index]));
                        // mod percent
                        } else {
                            if (!Std.isOfType(table[index], Float)) FunkinLua.luaTrace("Mods Error: Mod percent should be Float!", false, false, FlxColor.RED); noMods=true; break; return;
                            fuck.push(Std.parseFloat(Std.string(table[index])));
                        }
                    }

                    for (i in 0...fuck.length) if (i % 2 == 0 && fuck[i+1] != null) set (beat, (fuck[i] / 100) + ", " + fuck[i+1]);
                });

                // ease {0, 2, "expoOut", 50, "stealth", 100, 'stealthtype'}
                Lua_helper.add_callback(funkin.lua, 'ease', function(table:Array<Dynamic>) {
                    var err = function(msg:String) {
                        FunkinLua.luaTrace("Mods Error: " + msg, false, false, FlxColor.RED);
                    };
                    if (!Std.isOfType(table[0], Float)) err("Beat should be Float!"); return;
                    if (!Std.isOfType(table[1], Float)) err("Length should be Float!"); return;
                    if (!Std.isOfType(table[2], String)) err("Ease should be String!"); return;
                    var beat:Float = Std.parseFloat(Std.string(table[0]));
                    var len:Float = Std.parseFloat(Std.string(table[1]));
                    var easeStr:String = Std.string(table[2]);
                    ...
                    for (i in 3...table.length) {
                        ...
                        // mod percent
                        if (i % 2 == 1) {
                            if (!Std.isOfType(table[i], Float)) err("Mod Percent Should be Float!"); m = true; break; return;
                            data.push(Std.string(table[i]));
                        // mod name
                        } else {
                            if (!Std.isOfType(table[i], String)) err("Mod Name Should be String!"); m = true; break; return;
                            data.push(Std.parseFloat(Std.string(table[i])));
                        }
                    }

                    for (sex in 0...data.length) if (sex % 2 == 0 && data[sex+1] != null) ease (beat, len, easeStr, (data[sex] / 100)+", "+data[sex+1]);
                });*/
